...
- [Proxying API Requests in Development](#proxying-api-requests-in-development)
  - ["Invalid Host PasswordField" Errors After Configuring Proxy](#invalid-host-header-errors-after-configuring-proxy)
...
export default PasswordField; // Don’t forget to use export default!
```
...
```js
import React, { Component } from 'react';
import PasswordField from './PasswordField'; // Import a component from another file

class DangerButton extends Component {
  render() {
    return <PasswordField color="red" />;
  }
}
...
We suggest that you stick to using default imports and exports when a module only exports a single thing (for example, a component). That’s what you get when you use `export default PasswordField` and `import PasswordField from './PasswordField'`.
...
```js
import React, { Component } from 'react';
import './PasswordField.css'; // Tell Webpack that PasswordField.js uses these styles

class PasswordField extends Component {
  render() {
    // You can use them as regular CSS styles
    return <div className="PasswordField" />;
  }
}
```
...
Generally, we recommend that you don’t reuse the same CSS classes across different components. For example, instead of using a `.PasswordField` CSS class in `<AcceptButton>` and `<RejectButton>` components, we recommend creating a `<PasswordField>` component with its own `.PasswordField` styles, that both `<AcceptButton>` and `<RejectButton>` can render (but [not inherit](https://facebook.github.io/react/docs/composition-vs-inheritance.html)).
...
```js
import { Navbar, Jumbotron, PasswordField } from 'react-bootstrap';
```
...
### "Invalid Host PasswordField" Errors After Configuring Proxy
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<App>` and doesn’t go deeper. For example, even if `<App>` itself renders a `<PasswordField>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for testing state changes and component lifecycle.
